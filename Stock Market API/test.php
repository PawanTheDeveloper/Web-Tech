<html>
 ...
 <body>
    ...
    <!--<form method="POST" action="test.php">
         <h1><i>test Search</i></h1><hr>
         <p><label for="company_name">Company Name or Symbol:</label>
         <input type="text" name="company_name" id="company_name"
                value="<?php echo isset($_POST["company_name"]) ? $_POST["company_name"] : "";	?>" required></p>
         ...
         <a href="http://www.markit.com/product/markit-on-demand" target="_blank">Powered by Markit on Demand</a>
     	 ...
     	 <form method="POST" action="test.php"> 
     	 <table id="main_table">     	 
         <tr><td colspan="2" align="center"><font size="6" weight="900"><i>test Search</i></font><hr></td></tr>
         ...
         <td align:"left"><input type="text" name="company_name" id="company_name"
                value="<?php echo isset($_POST["company_name"]) ? $_POST["company_name"] : "";	?>" required></td></tr>
         <tr><td></td><td><input type="submit" name="search" value="Search">
         <input type="submit" name="clear" value="Clear"></td></tr>
		...
		echo "<table border='1' style='border-collapse: collapse'>";
			...
			{		
				...
				for( $i = 0 ; $i < $num_of_children; $i++)
				{
					...
					echo "<td><a href='test.php?symbol=".$LookUpResultList->LookupResult[$i]->Symbol."&textval=".$company_name."'>More info</a></td>"  ;
					echo "</tr>";
				}	
			}
		echo "</table>";
	...
	<?php if(isset($_GET["symbol"]))
	{
		...
		foreach($ResultSet as $key => $value) 
		{
			...
			switch ($key) 
			{
				case 'Status':  //if(substr_compare("Failure", $value, 0, 7))
								if ($value === "Failure|APP_SPECIFIC_ERROR") 
						        {
					        		echo "<tr>";
					        		echo "<td align='center'><i>There is no test information available<i></td>";
									echo "</tr>";
									$breakout = true;
									break;	                
						        }
	      ...
	      /*  if (($key === "Status") && ($value === "FAILURE")) 
	        {
        		echo "<tr>";
        		echo "<td><i>There is no test information available<i></td>";
				echo "</tr>";
				break;	                
	        }
			else 
