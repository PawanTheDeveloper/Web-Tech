	...
	if( $("#searchForm").valid() )
	{
		...
		{	
			...
			{	
				...
				$("#sName").html(output.Name);		
				
				$("#sSymbol").html(output.Symbol);
					...
					FB.ui(
					  {
						...
						title: 'Current Stock Price of '+output.Name+' is $'+output.LastPrice,
						description: 'Stock Information of '+ output.Name + ' (' + output.Symbol + ')',
						caption: 'LAST TRADE PRICE: ' + '$' + output.LastPrice + ', CHANGE: ' + finalChange + '(' + finalChangePercent + '%)' ,
...
function fav_tabletockValue(name)
{
	//alert(name);
	$("#searchVal").val(name);
	filtable();
...
{ 
	...
	{	
		...
		for(var i=0; i <= a; i++)
		{
			...
			success: function(output)
			{	
				content="<tr id='row" + i + "'><td align='left'><a href='#carouselpart'  data-slide-to='1' onclick='fav_tabletockValue("+ '"'+output.Symbol+'"' +"); return false;'>"+output.Symbol+"</a></td><td align='left'>";
				...
				//content = "<tr id='row" + i + "'><td align='left'><a href='javascript:fav_tabletockValue(" + output.Symbol + ")'>" + output.Symbol + "</a></td><td align='left'>";
				...
				{ 	
					...
					content += "<td align='left'><font color='red'>" + favoriteChange + "</style></td><td align='left'>";
				}					
				else
				{
					...
					content += "<td align='left'><font color='green'>" + favoriteChange + "</style></td><td align='left'>";
				}
				...
				if(marketmillion < 10)
				{
					var marketmillion1 = Math.round(marketmillion*100)/100;
					var marketmillion1Final = marketmillion1.toFixed(2);
					var marketCapMillionHtml = marketmillion1Final + " Million</td>";
					content += marketCapMillionHtml;
				}
				if(marketmillion > 10)
				{
					var marketcap1roundFinal = marketcap1round.toFixed(2);
					var marketCapInBillionHtml= marketcap1roundFinal + " Billion</td>";	
				...
				document.getElementById('fav_table').innerHTML += content;
			}
			});
		}
	}
